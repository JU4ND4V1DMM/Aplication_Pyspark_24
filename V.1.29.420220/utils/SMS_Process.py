...
def Function_Complete(path, Wallet_Brand, output_directory, Type_Proccess, Partitions):

    Data_Frame = First_Changes_DataFrame(path)
    Data_Frame = Phone_Data(Data_Frame)
    Data_Frame = SMS_Proccess(Data_Frame, Wallet_Brand, output_directory, Type_Proccess, Partitions)
...
def SMS_Proccess (Data_, Wallet_Brand, Directory_to_Save, Type_Proccess, Partitions):
    ...
    Data_ = Data_.filter(col("Dato_Contacto") >= 3000000000)
    Data_ = Data_.filter(col("Dato_Contacto") <= 3599999999)
