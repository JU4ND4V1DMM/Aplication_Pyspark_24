...
from pyspark.sql.functions import col, concat, lit, upper, regexp_replace
from pyspark.sql.functions import expr, when, row_number, collect_list
...
def Phone_Data_Div(Data_Frame):
    ...
    phone_list_columns = [f"phone_list{i}" for i in range(1, 7)]
    consolidated_data = (
        Data_Frame.groupBy("identificacion")
        .agg(*[collect_list(f"phone{i}").alias(f"phone_list{i}") for i in range(1, 7)])
    )
    Data_Frame = Data_Frame.join(consolidated_data, "identificacion")
...
def BOT_Process (Data_, Wallet_Brand, Directory_to_Save, Partitions):
    ...
    Data_ = Data_.select("identificacion", "cuenta", "cuenta2", "phone1", "phone2", "phone3", "phone4", "phone5", "phone6", \
                         "marca", "origen", f"{Price_Col}", "nombrecompleto", "referencia", "descuento", "Filtro", )
    
    Data_ = Data_.orderBy(["identificacion", "Filtro"])
