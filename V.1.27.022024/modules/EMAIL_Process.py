...
def Function_Complete(path, Wallet_Brand, output_directory):

    Data_Frame = First_Changes_DataFrame(path)
    Data_Frame = Email_Data(Data_Frame)
    Data_Frame = EMAIL_Proccess(Data_Frame, Wallet_Brand, output_directory)
...
def EMAIL_Proccess (Data_, Wallet_Brand, Directory_to_Save):
    ...
    Data_ = Data_.filter(col("Dato_Contacto") != "")
...
File_Name = "CORREOS LIMPIOS 22-02-24xlsx"
Wallet_Brand = "prechurn"
...
Function_Complete(path, Wallet_Brand, output_directory)