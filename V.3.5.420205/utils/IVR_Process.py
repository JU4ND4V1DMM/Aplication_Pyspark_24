...
def Phone_Data(Data_):
    ...
    columns_to_drop_contact = columns_to_stack
    stacked_contact_data_frame = Data_.select("*", *columns_to_stack)
...
def IVR_Process (Data_, Directory_to_Save, partitions, filter_brands, filter_origins):
    ...
    Data_ = Data_.withColumn(
        ...
        when(col("fechagestion_contactodirecto").isNull(), 0)
    ...
    Data_ = Data_.select("Dato_Contacto", "Telefono 2", "Telefono 3", "**", "identificacion", "origen", "**2", "cuenta", \
                         "marca", "fechagestion_contactodirecto", "fecha_asignacion", "fecha_vencimiento", "Mod_init_cta", \
                         "descuento")
    
    Data_ = Data_.dropDuplicates(["Dato_Contacto"])
    Data_ = Data_.orderBy(["Mod_init_cta", "origen","Dato_Contacto",'marca', "fechagestion_contactodirecto"])
